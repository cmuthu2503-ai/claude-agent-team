...
| Build Plan Decomposition (shipped 2026-05-23) | Three-level hierarchy (**Epic → Feature → Task**) + task-level dependency DAG layered on top of `project_tasks`. Replaces today's flat 30-40-task list with ~5-12 epics × 1-8 features per epic × 3-15 atomic tasks per feature (~150-200 total). Each task is BOUNDED: one `primary_file`, ≤2 additional files, 50-300 LOC, one `acceptance_test` sentence — closes the four CrewAI production failure classes (T-6144cc94 / T-103e9025 / T-b4954195 / T-3e1303b3, all of which were "task scope too large to deliver in one response"). Three-pass LLM generation with review gates: Pass 1 (PRD → epics) / Pass 2 (epic → features) / Pass 3 (feature → atomic tasks with depends_on indices). Pass 3 persistence runs `has_task_cycle` and rolls back the whole batch with 422 on cycle detection. Dispatch endpoints enforce `depends_on` — task can't fire until every blocker is `deployed`. Auto-dispatch handler (opt-in per project via `auto_dispatch_on_deploy`) recomputes the dispatchable set on each `request.deployed` and fires newly-unblocked tasks; emits `project.tasks.auto_dispatched` over WS. Frontend gains `BuildPlanGenerator` (3-pass UI + mega-button), `BuildPlanView` (3-level collapsible tree with per-row Generate / Dispatch actions and rollup chips), `BuildPlanRollupChip` (N/M epics · X/Y tasks · Z blocked), TaskDrillIn breadcrumb + atomic-task block + depends_on chips. Legacy projects (no epics, no BPD fields on tasks) work unchanged — `BuildPlanView` returns null, and any flat tasks render under a synthetic "Legacy" pseudo-epic when epics exist. Build Chat orchestrator gains 3 new tools (`dispatch_feature`, `dispatch_epic`, `dispatch_all_ready`). 34 of 45 BPD tasks done in this ship; 3 partial (Build Board filters, epic-detail popup, inline DAG editing) and 8 phase-E verification deferred to a Phase D2 / E follow-up. | Backend: `src/state/sqlite_store.py` (epics + features tables + 5 ALTER on project_tasks + 14 new CRUD methods + DAG helpers), `src/state/base.py` (18 new abstract methods), `src/models/base.py` (Epic + Feature + 5 BPD fields on ProjectTask + `auto_dispatch_on_deploy` on Project), `src/api/routes/projects.py` (+~1300 lines: 3 GET endpoints, 6 POST generators, 4 dispatch endpoints, 2 rollup endpoints, 1 decompose-legacy endpoint, shared `_check_dependencies_unmet` + `_dispatch_ready_tasks` helpers), `src/core/auto_dispatch.py` (new — event handler), `src/core/build_chat.py` (3 new chat tools), `src/agents/base.py` (surface `stop_reason` from `_call_anthropic` → `single_call`). Frontend: `frontend/src/components/projects/BuildPlanGenerator.tsx` (new — 280 LOC, 3-pass step buttons + orchestrator mega-button), `frontend/src/components/projects/BuildPlanView.tsx` (new — 420 LOC, 3-level collapsible tree + Legacy epic + popup wiring), `frontend/src/components/board/TaskDrillIn.tsx` (Hierarchy / Atomic Task / Depends On sections), `frontend/src/components/board/types.ts` (CardData.bpd extension), `frontend/src/pages/ProjectDetail.tsx` (BuildPlanRollupChip), `frontend/src/components/projects/BuildWorkspace.tsx` (mounts both new components). Docs: `docs/prd.md` v3.13 (Section 6.8 + revision history), `docs/prd-build-plan-decomposition.md` v1.0 (deeper design doc with worked CrewAI example), `docs/mockups/build-plan-decomposition.html` (full UI mockup), `docs/agent-lessons-learned.md` (L19 atomic-task contract), `docs/task-list.md` (this row + BPD-01–45 breakdown). Tests: `tests/test_bpd_schema_and_crud.py` (23), `tests/test_bpd_generation_helpers.py` (25), `tests/test_bpd_dispatch_and_rollups.py` (14), `tests/test_bpd_end_to_end.py` (4) — 66 BPD tests total. |
...
| ID | Task | Effort | Depends On | Status |
|----|------|--------|-----------|--------|
| BPD-38 | Frontend renders legacy tasks (feature_id IS NULL) under a synthetic "Legacy" epic at the top of the Task List page (BPD-402) | S | BPD-29 | `[x]` |
| BPD-39 | Optional migration tool: `POST /projects/{id}/build-plan/decompose-legacy` — runs three-pass generation against the existing PRD and shows a diff view ("here's how your existing tasks would map"). User reviews + accepts (BPD-403) | L | BPD-17, BPD-29 | `[x]` |
...
| BPD-41 | Smoke test: end-to-end on a fresh project — generate epics → review → generate features → review → generate tasks → review → dispatch one feature → verify deps enforce → enable auto-dispatch → verify cascade | L | BPD-35, BPD-24 | `[x]` |
| BPD-42 | Smoke test: legacy compatibility — verify CrewAI's existing task list dispatches identically to before (no `depends_on` = no blockers) (BPD-404) | S | BPD-20 | `[x]` |
| BPD-43 | PRD revision history bump: docs/prd.md v3.12 → v3.13 (this PRD update) | S | BPD-41 | `[x]` |
