...
| Status | COMPLETE — all 63 tasks shipped (P0–P4) |
...
| P4 | Autonomous-Loop Reconciliation | 6 | 6 | 0 | 0 | 0 |
| **Total** | | **63** | **63** | **0** | **0** | **0** |
...
| HAI-17 | Outbound bridge core | EventEmitter subscriber that forwards curated events to a configured webhook; soft-fail, retry/drop, never blocks broadcasting. **Done:** `src/core/push_bridge.py` `PushBridge`; `events.on(bridge.handler)` registered in `main.py` only when `PUSH_WEBHOOK_URL` set (else disabled → pull). | M | HAI-02 | FR-070, FR-071, NFR-002 | `[x]` |
| HAI-18 | Push event selection + payloads | Config-driven forwarding of the **three real existing events only** in P1 (`request.failed`, `request.completed`, `deploy_health.anomaly_detected`); concise payloads linking back to ids. **No `request.deployed`** (never emitted). `proposal.*` forwarding is deferred to P2 (HAI-61). **Done:** `DEFAULT_PUSH_EVENTS` (the 3) + `PUSH_EVENTS` override; `_build_payload` (source/event/request_id/summary/detail) + `X-Push-Secret`. | S | HAI-17 | FR-070, FR-072, FR-075 | `[x]` |
| HAI-19 | Pull-only fallback verify | Confirm Monitor tools + Hermes scheduler reconcile state with the bridge disabled. **Done:** bridge off by default (`push_bridge_disabled` boot log); `test_pull_only_baseline_no_bridge_registered` proves emit + non-push handlers run cleanly with no bridge; monitor tools are a separate module independent of push. | S | HAI-10..16 | FR-073 | `[x]` |
| HAI-54 | Bridge disconnect / bounded gap | Buffer failed deliveries with bounded retry/TTL; document pull (FR-073) as the durability backstop so any missed alert's gap ≤ pull interval. Never block EventEmitter. **Done:** handler is enqueue-only (bounded `asyncio.Queue`, drop-when-full); background worker delivers with bounded retry+backoff; pull documented as the durability backstop in setup-hermes-integration.md. 10 tests. | M | HAI-17 | FR-074 | `[x]` |
| HAI-20 | P1 tests + Hermes connect E2E | Unit tests for read tools/bridge; manual E2E: connect Hermes, run a brief, trigger a failure, see alert. **Done:** P1 suite green — 29 MCP (tools+manifest+contract+adapter), 12 push-bridge incl. emit→bridge→webhook integration, 22 auth surface. `test_emit_flows_through_bridge_to_webhook` is the automated "failure → alert" E2E. Operator verification checklist (live `hermes mcp test` + push) in setup-hermes-integration.md §7 (the live-Hermes connect is a manual operator step). | M | HAI-10..19, HAI-54 | NFR-004 | `[x]` |
...
| HAI-22 | Proposal model + store methods | Pydantic model + CRUD/transition methods on the state store. **Done:** `ProposalStatus` enum + `Proposal` model; store `create_proposal`/`get_proposal`/`get_proposal_by_idempotency_key`/`list_proposals`/`update_proposal` on the abstract + SQLite store. 6 tests. | M | HAI-21 | FR-030 | `[x]` |
| HAI-23 | `POST /proposals` (create) | Create pending proposal; no side effects; emit `proposal.created`. **Done:** `src/api/routes/proposals.py` POST (get_principal; proposed_by=`principal_actor`); idempotent on `idempotency_key` (FR-035a); emits `proposal.created`; registered in main.py. 4 route tests + live E2E: a service token creates a proposal (201) but is 403 on every other write. | M | HAI-22 | FR-031 | `[x]` |
...
| HAI-61 | `proposal.*` forwarding | Forward `proposal.created`/`proposal.expired` via the bridge (deferred from P1 — producers now exist). | S | HAI-23, HAI-29, HAI-17 | FR-070, FR-075 | `[x]` |
| HAI-62 | Gate observability | Queryable signals: pending-backlog depth, expired-without-action rate, service-token call volume. | M | HAI-28 | FR-083 | `[x]` |
| HAI-63 | In-process gate interception | Wrap internal handlers (`orchestrator.submit` from auto_dispatch, `auto_rollback` from ops_heal) so in-process callers also pass the gate — not just HTTP routes. | L | HAI-25 | FR-035 | `[x]` |
| HAI-32 | P2 tests | Coverage ≥80% on proposals engine + guard; explicit test that no gated path (HTTP **and in-process**) executes ungated; idempotency/CAS/recovery covered. | M | HAI-21..30, HAI-55..63 | NFR-004, M1 | `[x]` |
...
| ID | Task | Description | Effort | Depends On | Traces FR | Status |
|----|------|-------------|--------|-----------|-----------|--------|
| HAI-33 | `task_submit` (inferred project, parked) | Creates a `request.submit` proposal carrying Hermes's inferred `project_id` + rationale; nothing runs until confirm. | M | HAI-26, HAI-12 | FR-040, FR-041 | `[x]` |
| HAI-34 | Unassigned explicit-confirm path | No inference → proposal records `Unassigned`; operator must consciously confirm. | S | HAI-33 | FR-042 | `[x]` |
| HAI-35 | Service-submit forced through gate | Service-token submits route through proposals; human dashboard submit unchanged. | M | HAI-25 | FR-043, NFR-001 | `[x]` |
| HAI-36 | `project_create` | `project.create` proposal → `POST /projects`. | S | HAI-26 | FR-050 | `[x]` |
...
| ID | Task | Description | Effort | Depends On | Traces FR | Status |
|----|------|-------------|--------|-----------|-----------|--------|
| HAI-45 | Governed-mode flag plumbing | Config flags selecting legacy-auto vs propose mode; default propose when a Hermes operator identity exists, legacy otherwise. | M | HAI-24 | FR-063, NFR-001 | `[x]` |
...
| HAI-50 | P4 tests + audit query | Loop-mode tests; audit query proving zero ungated executions by the service principal (M1). | M | HAI-45..49 | NFR-004, M1, M5 | `[x]` |
